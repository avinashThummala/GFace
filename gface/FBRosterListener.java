...
public class FBRosterListener implements RosterListener
{
    ...
    FBRosterListener(FBChat ref, Roster roster)
    {
        fbChat=ref;
        this.roster=roster;
    }
    ...
    public void presenceChanged(Presence presence) {
        ...
        if(fbChat.friends.containsKey(jid))
        {
            if(!presence.isAvailable())
            {
                ...
                {
                    ((CMessageFrame)fbChat.chatWindows.get(jid)).dispose();
        ...
        else
        {
            ...
            {                
                ...
                fbChat.fPanel.addComp(jid, new FriendViewPanel((ChatFriendEntry)fbChat.friends.get(jid), fbChat, fbChat.user));                
                fbChat.cFrame.setVisible(false);
                fbChat.cFrame.repaint();
                fbChat.cFrame.setVisible(true);
            }
